...
void Renderer::InitializeRenderingInfo(GLuint program)
{
	...
	glm::mat4 VP = projection == ProjectionType::ScreenOrthographic ? Camera::Instance().GetProjectionMatrix(projection) : Camera::Instance().GetProjectionMatrix(projection) * Camera::Instance().GetViewMatrix();
